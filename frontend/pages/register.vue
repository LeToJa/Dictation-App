	...
	<NuxtLayout>
		<div class="min-h-screen flex items-center justify-center px-4 py-10">
			<div class="max-w-md w-full space-y-8 bg-white border border-gray-300 shadow-lg rounded-xl p-8">
				...
				<p class="text-center text-sm">Empieza con tu registro para crear tu primera transcripción</p>
				<div v-if="noticeMessage" :class="['rounded-md p-3 mb-4 text-sm', noticeType === 'success' ? 'bg-green-100 border border-green-300 text-green-800' : 'bg-red-100 border border-red-300 text-red-800']">
					{{ noticeMessage }}
				</div>
...
const handleRegister = async () => {
    ...
    try {
        ...
        setNotice('¡Registro exitoso! Redirigiendo a inicio de sesión en 5 segundos...', 'success')
        setTimeout(async () => {
            await navigateTo('/login')
        }, 5000)
    } catch (error) {
        console.error('Registro error:', error)
        setNotice('Error al registrarse: ' + (error || 'respuesta desconocida'), 'error')
    } finally {
        loading.value = false
    }
}
